...
def f_strain_spline3_smooth(alt, sp, t):
    ...
    N_abscisses = len(w_strain) - 3.0
    z = alt * N_abscisses / t
    index = 0
    strain = np.zeros(len(z))
    for i in z:
        strain[index] = cubicSpline(i, w_strain) / 100.0  ## LES POIDS SONT EN %
        index = index + 1
    return strain
...
def control_sp(alt, sp, t, model):
    ...
    if model == "B-splines smooth":
        z_sp = (
            np.arange(1, Nspline + 1) * t / Nspline
        )  # generate depth (x axis) for the strain basis function
        z_interp = np.arange(1, (Nspline * Nslice) + 1) * t / (Nspline * Nslice)
    if model == "B-splines abrupt":
        z_sp = (
            np.arange(0, Nspline) * t / (Nspline - 1)
        )  # generate depth (x axis) for the strain basis function
    ...
    scaled_sp = strain[
        np.in1d(np.around(z_interp, decimals=1), np.around(z_sp, decimals=1))
    ]
...
def old2new_strain(alt, sp, t, new_size, model):
    ...
    def errfunc(sp, alt, strain, t, model):
        return f_strain(alt, sp, t, model) - strain_old
